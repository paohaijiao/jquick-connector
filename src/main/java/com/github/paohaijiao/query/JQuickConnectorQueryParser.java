 ...
 * @since 2025/10/21
 */
public class JQuickConnectorQueryParser {
    private JContext context;

    public JQuickConnectorQueryParser(JContext context) {
        this.context = context;
    }

    public JQuickConnectorQueryParser() {
        this.context = new JContext();
    }

    public JQuickConnectorParsedQuery parse(String query) {
        JConsole console = new JConsole();
        JQuickConnectorExecutor executor = new JQuickConnectorExecutor(context);
        console.log(JLogLevel.INFO, "query: " + query);
        JQuickConnectorParsedQuery parsedQuery = executor.execute(query);
