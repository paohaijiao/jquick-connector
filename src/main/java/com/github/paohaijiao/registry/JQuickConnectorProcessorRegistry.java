 ...
 * @since 2025/10/21
 */
public class JQuickConnectorProcessorRegistry {

    private static final Map<String, JQuickConnectorProcessor> processors = new ConcurrentHashMap<>();

    private static final List<JQuickConnectorProcessor> prioritizedProcessors = new ArrayList<>();
     ...
     */
    public static void registerProcessor(String name, JQuickConnectorProcessor processor, int priority) {
        lock.writeLock().lock();
        try {
     ...
     * 根据名称或别名获取处理器
     */
    public static JQuickConnectorProcessor getProcessor(String name) {
        lock.readLock().lock();
        try {
            String normalizedName = resolveName(name);
            JQuickConnectorProcessor processor = processors.get(normalizedName);
            if (processor != null) {
                return processor;
            }
            for (JQuickConnectorProcessor p : processors.values()) {
     ...
     * 获取按优先级排序的处理器列表
     */
    public static List<JQuickConnectorProcessor> getPrioritizedProcessors() {
    ...
    public static void unregisterProcessor(String name) {
        ...
        try {
            String normalizedName = resolveName(name);
            JQuickConnectorProcessor removed = processors.remove(normalizedName);
            if (removed != null) {
