...
import com.github.paohaijiao.meta.JQuickConnectorMetadata;
...
public class JQuickConnectorRegistry {
    ...
    private static void initializeRegistry() {
        ...
        List<JQuickConnectorHandler> loadedHandlers = ServiceLoader.loadServices(JQuickConnectorHandler.class);
        for (JQuickConnectorHandler handler : loadedHandlers) {
     ...
     * 注册连接器
     */
    public static synchronized void registerConnector(String type, JQuickConnectorHandler connector) {
        checkRegistryState();
        ...
        JQuickConnectorMetadata metadata = new JQuickConnectorMetadata(normalizedType, connector);
    ...
    public static JQuickConnectorHandler getConnector(String type) {
        checkRegistryState();
        String normalizedType = normalizeType(type);
        JQuickConnectorMetadata metadata = CONNECTOR_METADATA_MAP.get(normalizedType);
     ...
     * 获取连接器元数据
     */
    public static JQuickConnectorMetadata getConnectorMetadata(String type) {
        checkRegistryState();
    ...
    public static synchronized boolean unregisterConnector(String type) {
        checkRegistryState();
        String normalizedType = normalizeType(type);
        JQuickConnectorMetadata metadata = CONNECTOR_METADATA_MAP.remove(normalizedType);
        if (metadata != null) {
    ...
    public static boolean containsConnector(String type) {
        checkRegistryState();
        JQuickConnectorMetadata metadata = CONNECTOR_METADATA_MAP.get(normalizeType(type));
    ...
    public static Set<String> getRegisteredTypes() {
        ...
        return Collections.unmodifiableSet(CONNECTOR_METADATA_MAP.values().stream()
                .filter(JQuickConnectorMetadata::isActive)
                .map(JQuickConnectorMetadata::getType)
     ...
     * 获取所有活跃的连接器
     */
    public static Collection<JQuickConnectorHandler> getActiveConnectors() {
        checkRegistryState();
        return Collections.unmodifiableList(
                CONNECTOR_METADATA_MAP.values().stream()
                        .filter(JQuickConnectorMetadata::isActive)
                        .map(JQuickConnectorMetadata::getHandler)
     ...
     * 根据条件查找连接器
     */
    public static List<JQuickConnectorHandler> findConnectors(Predicate<JQuickConnectorMetadata> predicate) {
        checkRegistryState();
     ...
     * 按使用次数排序获取连接器
     */
    public static List<JQuickConnectorHandler> getConnectorsByUsage() {
        checkRegistryState();
        return Collections.unmodifiableList(CONNECTOR_METADATA_MAP.values().stream()
                .filter(JQuickConnectorMetadata::isActive)
                .sorted(Comparator.comparingInt(JQuickConnectorMetadata::getUsageCount).reversed())
                .map(JQuickConnectorMetadata::getHandler)
                .collect(Collectors.toList()));
    }
     ...
     * 根据类型层次结构查找连接器
     */
    public static <T extends JQuickConnectorHandler> List<T> getConnectorsByType(Class<T> handlerType) {
        checkRegistryState();
        Objects.requireNonNull(handlerType, "Handler type cannot be null");
        List<JQuickConnectorMetadata> metadataList = TYPE_HIERARCHY_MAP.getOrDefault(handlerType, Collections.emptyList());
    ...
    public static Map<String, Object> getRegistryStats() {
        ...
        long totalUsage = CONNECTOR_METADATA_MAP.values().stream()
                .mapToInt(JQuickConnectorMetadata::getUsageCount)
                .sum();
    ...
    private static void updateTypeHierarchy(Class<?> handlerClass, JQuickConnectorMetadata metadata) {
        Class<?> currentClass = handlerClass;
        while (currentClass != null && JQuickConnectorHandler.class.isAssignableFrom(currentClass)) {
            TYPE_HIERARCHY_MAP.computeIfAbsent(currentClass, k -> new CopyOnWriteArrayList<>()).add(metadata);
            currentClass = currentClass.getSuperclass();
        }
    }

    private static void removeFromTypeHierarchy(JQuickConnectorMetadata metadata) {
    ...
    private static void notifyConnectorRegistered(String type, JQuickConnectorHandler handler) {
    ...
    private static void notifyConnectorUnregistered(String type, JQuickConnectorHandler handler) {
        LISTENERS.forEach(listener -> {
            try {
